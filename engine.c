...
/*
 * _00200046：RPG 游戏引擎主入口
 * - 初始化引擎状态（调用 _00200247）与播放启动特效（_00200B96）。
 * - 进入菜单循环，直到错误标志 MCU_memory[0x1935] 改变；在特定状态（0xFD）触发读取存档。
 * - 退出循环后调用错误处理（_002000C4），并进行一次系统内存池初始化。
 * 注意：MCU_memory[0x1935] 用作全局错误/状态码；0x00 表示无错误继续运行。
 */
void _00200046()
{
    // 初始化所有变量
    ...
    _002000C4(); // 发生错误时的处理
    __8EE9(0x07);
    SysMemInit(0x2C00, 0x1400); // 初始化系统内存池
}

/*
 * _002000C4：错误处理
 * - 当 MCU_memory[0x1935] 为非 0 且非特殊状态（FE/FD/FF）时，构造并显示 "RunErr:" + 错误码。
 * - 特殊状态（终止/暂停/重启）会清零错误码并直接返回。
 * - 显示完成后调用 __8F16（可能为界面/流程的后续处理）。
 */
void _002000C4()
{
    UINT8 _17C1[0x14]; // 用于存储错误信息字符串
    UINT8 _17BC[6]; // 用于存储转换后的错误代码
    UINT8 _742E[] = "RunErr:";
    if (MCU_memory[0x1935] == 0x00)
    {
        return;
    }
    // 检查是否为特殊错误状态（终止、暂停、重启）
    if (MCU_memory[0x1935] == 0xFE || MCU_memory[0x1935] == 0xFD || MCU_memory[0x1935] == 0xFF)
    {
        MCU_memory[0x1935] = 0x00;
        return;
    }
    fillmem(_17C1, 0x0014, 0x00); // 构建错误信息字符串
...
/*
 * _00200247：游戏初始化
 * - 初始化 GUI、Flash、内存池与图标；记录键盘状态并设置输入法过滤与键盘类型；关闭按键音并开启定时器。
 * - 执行数据 Bank 切换与游戏名读取；清零全局状态区域与敌人/角色/NPC/事件数据结构；初始化随机数。
 * - 分配多块运行缓冲（失败则写入 MEM_ERR 到 MCU_memory[0x1935]）。
 * - 设置默认功能开关：音乐开、可存档、关闭自动行走、取消坐标显示、定时事件默认关闭。
 */
void _00200247()
{
    UINT8 _17CC;
...
/*
 * _0020117E：返回存档位置标号（0,1,2）
 * - 行为：绘制存档槽UI，显示角色/标题等信息，处理方向与选择输入，返回选择的槽位索引。
 * - 返回：0..2 有效槽位；0xFF 表示错误/中断（`MCU_memory[0x1935]` 非0）。
 * 注意：依赖多处资源调用 `__8F0A/__8F04` 与消息系统；涉及屏幕保存/反色高亮与定时器管理。
 */
// 返回存档位置标号，返回值0,1,2
...
/*
 * _00203591：购买列表（BUY "t0 t1 ... tsum"）
 * - 参数：紧随其后的类型/索引对序列，最后以0结束（tsum为终止符）。
 * - 行为：解析物品类型与索引收集到数组，调用 `__8EC4(0, count, list)` 打开购买界面。
 * 注意：遍历直到遇到0终止；对齐与越界需由上层脚本保证。
 */
// BUY "t0 t1 ... tsum"
void _00203591()
{
...
/*
 * _0020378B：检测金钱不足跳转（TESTMONEY money lessadr）
 * - 参数：money 所需金额，lessadr 不足时跳转地址。
 * - 行为：若玩家持有金钱 `< money`，则将脚本偏移设为 `lessadr`。
 */
...
/*
 * _002038D3：调用章节（CALLCHAPTER m n）
 * - 参数：章 m、节 n。
 * - 行为：调用 `__8F40(m, n)` 切换并执行指定章节脚本。
 */
// CALLCHAPTER m n
void _002038D3()
{
...
/*
 * _002039BF：删除道具（DELETEGOODS type index erradr）
 * - 参数：type/index 指定道具；`erradr` 失败跳转地址。
 * - 行为：组包 `[type,index,0x63]` 调用 `__8ECD` 扣除（0x63表示删除任意数量）；失败则跳转到 `erradr`。
 */
...
/*
 * _00203B8B：显示消息（MESSAGE "..."）
 * - 参数：以0结尾的字符串；偏移基于 0x1AA0 与 0x1AA2。
 * - 行为：读取字符串并调用 `GuiMsgBox` 显示；不设超时。
 */
// MESSAGE "..."
void _00203B8B()
{
...
/*
 * _00204000：GUT 指令解释器（按偏移执行）
 * - 输入 `_17D0` 为当前章节内的命令偏移量；根据命令字节分派至 __8Dxx 处理函数。
 * - 首先装载当前章节的 GUT 数据到内存，记录对应 Bank，并设置脚本起始指针与偏移。
 * - 循环执行直到达到章节数据末尾或遇到控制指令（跳转/结束/错误）。
 * - 异常：当遇到未知指令时，使用 "错误指令..." 字符串提示，写入错误状态并返回。
 * 返回：状态码，表示继续、跳转或终止，具体由上层调度处理。
 * 依赖：DAT_GUDSCRIPT、MCU_memory[0x1AA0..1AA2] 作为脚本缓冲与偏移；DataBank 切换影响资源访问。
 */
// 执行对应偏移量的指令
...
/*
 * _002046A5：状态重置与后续处理
 * - 清理当前 GUT 缓存与相关状态区（1AA7+0x00C8 起的 0x38 字节）。
 * - 调用 `__8EE9(0x01)` 执行后续资源/流程处理（可能与存档或场景切换相关，依实现而定）。
 * 注意：对固定地址的清零依赖全局内存布局；建议确保 0x1AA7 已初始化。
 */
void _002046A5()
{
    __8DBA();
...
/*
 * _00204739：缓存 GUT 数据并初始化偏移
 * - 将当前章节的 GUT 数据（DAT_GUDSCRIPT）装载到内存，设置起始指针 0x1AA2。
 * - 复制命令偏移量表到 0x1AAE，设置当前命令偏移 0x1AA0 = 表项*2+3。
 * - 当全局错误 `MCU_memory[0x1935]` 非 0 时直接返回。
 * 注意：频繁递增 `*(UINT16*)(MCU_memory+0x1AA0)` 时需防止越界；偏移表大小为 0x201 字节。
 */
// 缓存GUT数据，并初始化GUT数据起始位置和GUT命令偏移量数据
void _00204739()
{
...
/*
 * _0020484C：加载地图（LOADMAP m n x y）
 * - 解析四个参数：地图组 m、地图索引 n、初始坐标 x/y（通过 0x1AA0+0x1AA2 指针读取）。
 * - 调用 `__8DD6(m,n,x,y)` 切换地图并定位；随后 `__8D96()` 刷新界面，`__8D99()` 重绘层，
 *   最后用 `__8DAE(MCU_memory + *(UINT16*)(MCU_memory+0x1936))` 更新消息或脚本位置。
 * 注意：多次递增 0x1AA0 指针，请确保命令缓冲边界安全；参数为单字节。
 */
// LOADMAP m n x y
void _0020484C()
{
...
/*
 * _00204A08：设置屏幕视口（SCREENS x y）
 * - 读取 x、y 参数并更新 `MCU_memory[0x197C/0x197D]` 作为屏幕与地图的对齐坐标。
 * - 影响后续绘制与坐标换算；典型用于场景切换或滚动策略变更。
 * 注意：仅更新左上角定位；若与地图边界不匹配可能出现越界访问风险。
 */
// SCREENS x y
void _00204A08()
{
...
/*
 * _00204B52：设置可控角色（SETCONTROLID actor）
 * - 参数：actor 角色ID。
 * - 行为：更新当前可控制角色，并调用 `__8DF1(actor)` 同步相关状态。
 */
// SETCONTROLID actor
void _00204B52()
{
...
/*
 * _00204BE5：删除角色（DELETEACTOR actor）
 * - 参数：actor 角色ID。
 * - 行为：`__8DFD(actor)` 校验存在后，调用 `__8DDC(actor)` 执行删除。
 * 注意：若角色不存在则直接返回。
 */
// DELETEACTOR actor
void _00204BE5()
...
/*
 * _00204C97：创建宝箱/容器（CREATEBOX id boxindex x y）
 * - 参数：id 实体ID，boxindex 宝箱资源索引，x/y 坐标。
 * - 行为：调用 `__8DD9(id-1, 0x04, boxindex, x, y)` 在场景中放置交互对象。
 */
...
/*
 * _00204E3D：删除宝箱（DELETEBOX id）
 * - 行为：日志打印后调用 `__8D2D()` 执行删除/刷新。
 */
// DELETEBOX id
void _00204E3D()
{
...
/*
 * 背包/道具布局（概要）
 * - 当前玩家索引：MCU_memory[0x1A94]
 * - 道具种类计数：MCU_memory[0x1A95]
 * - 每个角色状态块：起始 0x1988，步长 0x19；其中：
 *     - +0x01：角色资源ID
 *     - +0x17：该角色背包的起始偏移（指向三元组数组）
 * - 背包三元组条目：按顺序存放 [type, index, count]，每个 3 字节；
 *   例如第 i 个条目的计数位于：base + i*3 + 2。
 * - 金钱：MCU_memory[0x1A8F..0x1A92]（4 字节，低端序）
 * - 关键操作：
 *     - __8ED0(type,index)：查找道具在背包的条目索引，找不到返回 0xFF
 *     - __8ECA([type,index,num])：尝试加入背包（叠加或新增条目）
 *     - __8ECD([type,index,num])：从背包删除/消耗指定数量
 *
 * _00204E49：获得道具（GAINGOODS type index）
 * - 参数：`type` 道具类型，`index` 资源索引；数量固定为 1。
 * - 流程：调用 `__8ECA` 试图加入背包，失败则提示“已满载！”；成功则从 `DAT_GOODSRES` 读取道具资源，
 *   拼接“获得:” + 道具名称并显示提示框。
 * 注意：依赖背包容量与道具叠加逻辑；错误状态 `MCU_memory[0x1935]` 非 0 时直接返回。
 */
...
/*
 * _00205121：初始化战斗（INITFIGHT f0..f7 scrb scrl scrr）
 * - 参数：8 个敌人/阵型/概率等战斗参数，后跟 3 个屏幕滚动边界（左/右/底）。
 * - 流程：读取 11 个 16 位参数，记录并设置战斗初始化标志与可能出现的敌人种类数等。
 * 注意：参数读取使用 `_SysMemcpy`；请确保脚本缓冲足够且偏移正确。
 */
...
/*
 * FIGHTENABLE：开启野外遇敌（随机战斗）
 * - 将 `MCU_memory[0x1A96]` 置 1，使地图上允许触发随机战斗。
 */
// FIGHTENABLE
void _0020541C()
{
...
/*
 * FIGHTDISENABLE：关闭野外遇敌
 * - 将 `MCU_memory[0x1A96]` 置 0，禁用随机战斗。
 */
// FIGHTDISENABLE
void _00205443()
{
...
/*
 * ENTERFIGHT：进入指定配置的战斗
 * - 参数：
 *   - `roundmax` 最大回合数（写入 0x191D）
 *   - `f[3]` 敌方角色资源ID三元组，写入敌方阵列：基址 0x1826，步长 0x33，每个实体偏移 +0x01
 *   - 三个屏幕素材：大战斗背景(0x18DD)、左下角图(0x18DF)、右上角图(0x18DE)
 *   - `evtrnd[3]` 触发事件的回合（写入 0x191F..0x1921）
 *   - `evt[3]` 对应事件号（写入 0x1922..0x1924）
 *   - `lossto` 败北跳转地址、`winto` 胜利跳转地址
 * - 逻辑：设置上述内存后，调用 `__8E00(0)` 进行战斗流程；
 *   返回值 0x02 表示失败则将脚本指针 0x1AA0 设为 `lossto`；0x01 表示胜利则跳至 `winto`。
 */
...
/*
 * _00206D95：显示对话（SHOWGUT toppic botpic "text"）
 * - 参数：顶部/底部立绘索引 `toppic`/`botpic` 与后续的对话文本字符串。
 * - 流程：解析两个图片索引和一段以 0 结尾的字符串，调用 `__8D6F` 绘制立绘并显示文字气泡。
 * 注意：字符串读取使用 `strlen` 增量更新 0x1AA0；需防止越界与编码问题（假定单字节编码）。
 */
...
/*
 * _00206F18：消耗指定数量道具（USEGOODSNUM type index num erradr）
 * - 参数：`type`、`index`、`num`、`erradr`。
 * - 数据定位：
 *   1) 通过 `__8ED0(type,index)` 查找对应条目索引 i；若返回 0xFF 表示未拥有该道具。
 *   2) 读取当前玩家的背包基址：`base = *(UINT16*)(MCU_memory + 0x1988 + actor*0x19 + 0x17)`；
 *      其中 `actor = MCU_memory[0x1A94]`。
 *   3) 当前条目数量位于 `MCU_memory[base + i*3 + 2]`。
 * - 行为：数量不足则将脚本偏移设为 `erradr`；足够则组包 `[type,index,num]` 调用 `__8ECD` 扣减。
 * - 风险：需确保 `base` 合法且条目索引不越界；背包满/叠加规则由 `__8ECA/__8ECD` 内部约束。
 */
...
/*
 * _002071F7：随机跳转（RANDRADE rade trueadr）
 * - 参数：`rade` 为 0..999 的概率阈值，`trueadr` 为命中时跳转的脚本地址。
 * - 流程：生成 0..999 的随机数，与 `rade` 比较；若命中，将 `*(UINT16*)(MCU_memory+0x1AA0)` 设为 `trueadr`。
 * 依赖：随机环境 `MCU_memory+0x1928`。
 */
...
/*
 * _00207376：条件比较跳转（DISCMP n m lowaddr higaddr）
 * - 参数：从 0x1AA7+n 取值与 `m` 比较；小于则跳到 `lowaddr`，否则跳到 `higaddr`。
 * - 流程：读取参数并根据条件更新脚本偏移地址。
 * 注意：0x1AA7 区域为自定义状态寄存区；确保索引 n 不越界。
 */
...
/*
 * _0020758A：返回（RETURN）
 * - 行为：将 `MCU_memory[0x1AA4]` 置为 0x02 表示脚本返回；用于结束当前调用栈层级。
 */
// RETURN
void _0020758A()
{
...
/*
 * _00207590：定时消息（TIMEMSG time "..."）
 * - 参数：time 显示时长（单位10ms），后接以0结尾的字符串。
 * - 行为：读取 time 与字符串，调用 `GuiMsgBox` 显示后按时退出。
 */
// TIMEMSG time "......"
void _00207590()
{
...
/*
 * _002076D3：禁用存档（DISABLESAVE）
 * - 将 `MCU_memory[0x1A9B]` 置 0，表示当前不可存档；常用于剧情或战斗期间。
 * - 递增命令偏移指针 0x1AA0 跳过参数字节（此指令参数占 1 字节）。
 */
// DISABLESAVE
void _002076D3()
{
...
/*
 * _002076FA：启用存档（ENABLESAVE）
 * - 将 `MCU_memory[0x1A9B]` 置 1，允许玩家进行存档操作。
 * - 递增命令偏移指针 0x1AA0 跳过参数字节。
 */
// ENABLESAVE
void _002076FA()
{
...
/*
 * _00207721：执行存档（GAMESAVE）
 * - 跳过指令参数后调用 `__8EEC()` 执行具体存档流程（写入文件、序列化内存结构等）。
 * - 实际保存的内容与文件名规则由 `__8EEC` 与文件系统实现决定。
 * 注意：若 `MCU_memory[0x1A9B]` 为 0，则上层可能提前拦截并提示“目前不能存档”。
 */
// GAMESAVE
void _00207721()
{
...
// 播放特效/触发某类系统效果。_17CF:特效类型。_17D2:特效序号。_17D1:bit0代表是否支持按键跳过动画,bit1代表是否清空背景。_17D2:特效x坐标。_17D3:特效y坐标。
...
// 在固定UI区域打印数值。_17CF:x,_17D0:y,_17D1:数值,_17D5:0-小字体，1-正常字体
...
/*
 * _0023A64B：播放音乐（MUSIC n timers）
 * - 参数：`n` 音乐编号，`timers` 次数（0 表示关闭；0xFF 表示循环）。
 * - 行为：设置全局音乐编号与持续次数；若编号为 0 或全局音乐关闭则停止，否则调用 `SysPlayMelody(n)`。
 * 注意：`SysPlayMelody/SysStopMelody` 为占位实现，具体音频输出需平台支持。
 */
// MUSIC n	timers
void _0023A64B()
{
...
/*
 * USEGOODS：消耗单个道具（type/index，数量固定 1），失败跳转到 erradr。
 * - 逻辑：直接调用 `__8ECD([type,index,0x01])`；返回 0 则设脚本偏移为 `erradr`。
 */
...
/*
 * _002461F9：计算角色数据块长度
 * - 组成：基础信息、姓名、属性、装备、魔法链等段的长度累加。
 * - 用途：存档/读档序列化角色数据时用于确定固定块大小。
 * 返回：总长度（字节）。
 */
UINT8 _002461F9()
{
...
/*
 * _00246255：查找存档文件
 * - 输入：`_17CF` 为存档槽位索引（0..9）；`filename` 输出文件名；`filetype` 文件类型号。
 * - 根据当前游戏名和槽位号拼接存档文件名，查询文件系统是否存在。
 * - 返回：1 找到；0 未找到。
 * 注意：依赖 FileNum 等文件系统接口；需留意文件数量限制与路径规则。
 */
// 查找存档文件，找到返回1，没找到返回0
