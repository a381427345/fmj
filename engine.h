 ...
 *
 * 说明：
 * - 映射宏（__8Dxx/__8Exx）：将剧情脚本中的操作码映射到 engine.c 中的具体实现函数（形如 _002xxxxx），供脚本解释器进行命令分派。
 * - 指令常量（MUSIC/LOADMAP/...）：定义 GUT 脚本的内建命令编号，解释器根据编号选择执行路径；具体语义请参考下方对应函数原型的中文说明。
 * - 错误码与资源类型常量：统一错误与数据界类型的编码，便于在运行时进行判断与提示。
 *
 * 使用注意：
 * - 映射宏仅用于命令分派，不直接对外暴露业务语义；扩展新指令时，需要在此处添加宏映射，并在 engine.c 中提供实现。
 * - 指令常量应与解释器的 case 分派保持一致，避免编号偏移导致的运行时错误。
 */
/* 映射表：GUT脚本操作码 → 具体实现函数 */
...
/* 数据界类型常量：用于 Bank 切换与资源解析（与 middle.c 的 DataBankSwitch/GetDataBankNumber 协同） */
...
/* 脚本命令枚举：GUT 脚本的内建命令编号（具体语义由下方函数原型与 engine.c 实现说明） */
#define MUSIC             0
#define LOADMAP           1
...
void _00200046(); // RPG游戏引擎主入口函数
void _002000C4(); // 错误处理函数
void _00200247(); // 游戏初始化函数
void _00200942(UINT8 _17CF); // 释放资源函数
// 播放游戏开始动画
void _00200B96();
void _00200C79(); // 引擎菜单处理
// 游戏开始时的菜单，返回选择了哪项
UINT8 _00200DF1();
UINT8 _002010D5(); // 获取菜单项总数
// 读取进度界面
UINT8 _0020117E(UINT8 _17CF);
UINT8 _002018C0(); // 检查存档是否存在
UINT8 _002018C6(); // 读取存档函数
UINT8 _00201948(); // 读取存档函数(带确认)
void _00201A53(); // 加载角色图片资源
void _00201C6C(); // 加载玩家角色图片资源
void _00201EB2(); // 加载地图相关数据
UINT8 _00201F1E(UINT8 _17CF, UINT8 _17D0, UINT8* _17D1); // 选择对话框函数
void _0020239E(UINT8 _17CF, UINT8 _17D0); // 从指定剧情开始
void _0020249B(UINT8 _17CF); // 处理场景对象或NPC角色
void _002024FE(UINT8 _17CF); // 处理场景对象
void _00202C6D(UINT8 _17CF); // 处理NPC角色
UINT8* _00202F9F(UINT8 _17CF, UINT8* _17D0, UINT8* _17D2, UINT16* _17D4, UINT8* _17D6); // 获取角色图片数据
void _00203425(UINT8* _17D0, UINT8 _17D2, UINT8 _17D3, UINT8 _17D4); // 显示消息函数
// BUY "t0 t1 ... tsum"
// 买卖，t0 ... tsum为物品类型，每种以空格分开，在""中(最多20种)
void _00203591(); // 买卖道具函数
// TESTMONEY money lessadr
// 检测玩家金钱，如果金钱少于money，即跳到lessadr处执行
void _0020378B(); // 测试金钱函数
// CALLCHAPTER m n
// 调用号为n，类型为m的脚本，当前脚本执行位置被保存，所有资源不初始化；当被调用脚本CALLBACK时，返回调用位置继续执行。（注意：调用前后资源都不进行初始化，被调用脚本也会影响调用后的资源状况）
void _002038D3(); // 调用章节函数
// DELETEGOODS type index erradr
// 删除物品链中的物品（type，index），如果物品链中无此物品即跳到erradr处执行
void _002039BF(); // 删除道具函数
// MESSAGE "..."
// 用消息框显示"..."中信息（每行最多显示8个汉字，最多可显示5行），等待按键才返回。
void _00203B8B(); // 消息框函数
UINT8 _00204000(UINT16 _17D0); // 执行GUT脚本函数
// 处理对应NPC战斗触发事件相应的事件
UINT8 _00204563(UINT8 _17CF);
void _002046A5(); // 初始化GUT数据
void _00204739(); // 缓存GUT数据，并初始化GUT命令偏移量数据
// LOADMAP m n x y
// 载入号码n,类型m的地图，初始位置（x，y）
void _0020484C(); // 加载地图函数
// SCREENS x y
// 屏幕左上角定位到地图的（x，y）处
void _00204A08(); // 屏幕定位函数
// SETCONTROLID actor
// 设置角色号actor为当前控制(默认的被控制角色——游戏的主人翁的id=1)
void _00204B52(); // 设置控制角色函数
// DELETEACTOR actor
// 主角角色离队，从角色队列中删除指定主角号码actor的角色
void _00204BE5(); // 删除主角函数
// CREATEBOX id boxindex x y
// 创建一个宝箱，宝箱号码boxindex，位置为（x，y），id为操作号
void _00204C97(); // 创建宝箱函数
// DELETEBOX id
// 删除操作号为id的宝箱
void _00204E3D(); // 删除宝箱函数
// GAINGOODS type index
// 获得物品，类型为type，号码为index
void _00204E49(); // 获得物品函数
// INITFIGHT f0 f1 f2 f3 f4 f5 f6 f7 scrb scrl scrr
// 初始化随即战斗参数并开启随机战斗，f0-f7为可能出现的敌人的种类，scrb为大战斗背景，scrl为左下角图，scrr为右上角图
void _00205121(); // 初始化战斗函数
// FIGHTENABLE
// 开启随机战斗
void _0020541C(); // 开启随机战斗函数
// FIGHTDISENABLE
// 关闭随机战斗
void _00205443(); // 关闭随机战斗函数
...
void _0020546A(); // 进入战斗函数
// GAINMONEY money
// 得到数目为money的钱
void _00205B30(); // 获得金钱函数
// USEMONEY money
// 用去（丢失）数目为money的钱
void _00205C82(); // 使用金钱函数
// SETMONEY money
// 设置现钱数目为money
void _00205DD4(); // 设置金钱函数
// LEARNMAGIC actor type index
// 号码actor的主角学会法术类型为type，序号为index的法术
void _00205ED6(); // 学习魔法函数
// SALE
void _0020608A(); // 出售道具函数
// NPCMOVEMOD id mode
// 配角的运动模式，id为操作号，mode为 1 时随机运动，为 0 时原地不动,为 4 激活状态——只换图片，不改变位置（适合动态的物品对象）
void _002060B7(); // NPC运动模式函数
// RESUMEACTORHP id rade
// 恢复角色id生命rade%(一般用于剧情战斗失败,而游戏继续)
void _002062BB(); // 恢复角色生命函数
// DELALLNPC
// 删除当前场景所有NPC
void _0020663B(); // 删除所有NPC函数
...
void _00207590(); // 时间消息函数
// DISABLESAVE
// 禁止存档功能
void _002076D3(); // 禁止存档函数
// ENABLESAVE
// 允许存档功能
void _002076FA(); // 允许存档函数
// GAMESAVE
// 调用存档功能
void _00207721(); // 游戏存档函数
...
void _0020774E(); // 测试道具数量函数
// CREATEACTOR actor x y
// 创建主角号码actor，位置为（x，y）
void _00208000(); // 创建主角函数
// CREATENPC id npc x y
// 创建配角号码npc，位置为（x，y），id为操作号
void _0020813F(); // 创建NPC函数
// DELETENPC id
// 删除操作号为id的配角
void _002082CD(); // 删除NPC函数
// MOVE id x y
